...
class StatisticalAnalyzer:
    ...
    def _calculate_tanimoto_similarity_all(self, smiles_train: List[str], 
        ...
        try:
            ...
            if len(train_fps) * len(test_fps) > 1e6:  # Over 1M comparisons
            ...
            # Force sampling for comparisons over 1M
    ...
    def evaluate_split_similarity(self, X_train: np.ndarray, X_test: np.ndarray,
                                 smiles_train: List[str], smiles_test: List[str],
                                 split_type: str) -> Dict:
        """Evaluate similarity metrics based on split type (2025 update)"""
        print(f"    Evaluating similarity for {split_type} split...")
        
        # Calculate basic similarity metrics
        similarity_metrics = self.calculate_similarity_metrics(
            X_train, X_test, smiles_train, smiles_test
        )
        
        # Add split-specific evaluation
        if 'tanimoto' in similarity_metrics:
            tanimoto_mean = similarity_metrics['tanimoto']['stats']['mean']
            
            # Expected similarity ranges for different split types
            expected_ranges = {
                'random': (0.35, 0.65),  # Moderate diversity expected
                'temporal': (0.40, 0.70),  # Similar to random
                'scaffold': (0.20, 0.50),  # Lower similarity expected (different scaffolds)
                'cluster': (0.25, 0.55),   # Lower similarity expected (different clusters)
                'source': (0.30, 0.60),    # Moderate diversity
                'property': (0.15, 0.45),  # Low similarity (different property ranges)
                'experimental': (0.35, 0.65),  # Similar to random
                'kennard_stone': (0.30, 0.60),  # Uniform coverage
                'activity_cliff': (0.60, 0.90),  # High similarity expected
                'sali': (0.50, 0.80)  # Moderate to high similarity
            }
            
            split_evaluation = {
                'split_type': split_type,
                'mean_tanimoto': tanimoto_mean,
                'expected_range': expected_ranges.get(split_type, (0.30, 0.70)),
                'is_appropriate': False,
                'evaluation': ''
            }
            
            # Check if similarity is in expected range
            min_expected, max_expected = split_evaluation['expected_range']
            
            if tanimoto_mean < min_expected:
                split_evaluation['evaluation'] = f'Lower than expected ({tanimoto_mean:.3f} < {min_expected:.3f})'
                split_evaluation['is_appropriate'] = True if split_type in ['scaffold', 'cluster', 'property'] else False
            elif tanimoto_mean > max_expected:
                split_evaluation['evaluation'] = f'Higher than expected ({tanimoto_mean:.3f} > {max_expected:.3f})'
                split_evaluation['is_appropriate'] = True if split_type in ['activity_cliff', 'sali'] else False
            else:
                split_evaluation['evaluation'] = f'Within expected range ({min_expected:.3f} - {max_expected:.3f})'
                split_evaluation['is_appropriate'] = True
            
            # Add recommendations
            recommendations = []
            if not split_evaluation['is_appropriate']:
                if tanimoto_mean > 0.7 and split_type not in ['activity_cliff', 'sali']:
                    recommendations.append('High similarity - consider scaffold or cluster split for better diversity')
                elif tanimoto_mean < 0.3 and split_type not in ['scaffold', 'cluster', 'property']:
                    recommendations.append('Low similarity - good chemical diversity achieved')
            
            split_evaluation['recommendations'] = recommendations
            similarity_metrics['split_evaluation'] = split_evaluation
