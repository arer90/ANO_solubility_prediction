...
try:
    ...
    print("[WARNING] SciPy not available. Some statistical functions may be limited.")
...
class StatisticalVisualizer:
    ...
    def _plot_distribution_analysis(self, ax, splits: Dict):
        ...
        ax.axvline(mu - 2*sigma, color='orange', linestyle='--', label='±2sigma (95%)')
    ...
    def _plot_outlier_analysis(self, ax, statistical_results: Dict):
        ...
        if outlier_stats:
            headers = ['Dataset', 'IQR Method', 'Z-Score (>3sigma)', 'MAD Method']
...
* {'[OK]' if cv_percent < 30 else '[X]'} CV% < 30% (FDA/EMA bioanalytical requirement)
* {'[OK]' if outlier_percent < 5 else '[X]'} Outliers < 5% (data quality indicator)
...
STATISTICAL QUALITY:
* {'[OK]' if cv_percent < 30 else '[X]'} Low variability (CV% < 30%)
* {'[OK]' if outlier_percent < 5 else '[X]'} Few outliers (<5%)
* {'[OK]' if total_count > 500 else '[X]'} Sufficient sample size (>500)
...
4. DRUG DEVELOPMENT IMPLICATIONS:
   ...
   * Quality metrics guide go/no-go decisions"""
    ...
    def _plot_drug_development_insights(self, ax, splits: Dict):
        ...
        if developable > 70:
            recommendation = "[OK] High potential for drug development"
            color = "lightgreen"
        elif developable > 50:
            recommendation = "⚠ Moderate potential, formulation needed"
            color = "lightyellow"
        else:
            recommendation = "[X] Low potential, significant challenges"
            color = "lightcoral"
