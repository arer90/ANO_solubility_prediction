# ad_performance_analysis.py (SVD error fixed version)
...
class ADPerformanceAnalyzer:
    ...
    def __init__(self, output_dir: Path, n_jobs: int = -1, 
        ...
        # Performance directory disabled - not being used
        # self.performance_dir.mkdir(parents=True, exist_ok=True)
        
        # Remove by_split and overall directory creation - directly use performance_dir
        # Folders will be created as needed in the performance_dir
    ...
    def analyze_ad_performance_all_models(self, X_train: np.ndarray, y_train: np.ndarray, 
        ...
        if len(X_test) != len(y_test):
            print(f"  [ERROR] Test data size mismatch: X_test={len(X_test)}, y_test={len(y_test)}")
            return results
    ...
    def _analyze_single_model(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        # Save results directly in performance directory with mode and dataset structure
        save_dir = self.performance_dir / ad_mode / f"{dataset_name}_{split_type}"
    ...
    def _vectorized_ad_analysis(self, ad_results: Dict, y_test: np.ndarray,
        ...
        # Overall metrics (keep existing code)
        ...
        for method, ad_data in ad_results.items():
            if ad_data and 'in_ad' in ad_data:
                ...
                if len(in_ad_mask) != len(y_test):
                    ...
                    # Resize - use copy to protect original data
                    ...
                    # Recalculate all metrics with adjusted data
...
"""
        ...
        if mean_rmse_ratio < 1.5 and mean_coverage > 0.8:
            summary_text += "[OK] Excellent AD definition!\n"
            ...
            summary_text += "[OK] Good AD definition.\n"
    ...
    def create_combined_report(self, all_results: Dict[str, Dict[str, pd.DataFrame]], 
        ...
        print(f"\n[CHART] Creating combined AD performance report for {ad_mode} mode...")
        
        # Create a single combined report in the performance directory
        save_dir = self.performance_dir / ad_mode
        save_dir.mkdir(parents=True, exist_ok=True)
        
        # Create overall combined report directly
        try:
        ...
        print(f"  [OK] Combined report created for {ad_mode} mode")
...
"""
        ...
        print(f"    [OK] {split_type.upper()} split report saved")
...
"""
        ...
        print(f"  [OK] Overall combined report saved for {ad_mode} mode")