...
class MetaVisualizer:
    ...
    def _plot_drug_likeness(self, ax, name: str, features: Dict):
        ...
        # Veber's Rules
        # Get descriptor count at the beginning of the method
        n_descriptors = descriptors.shape[1] if len(descriptors.shape) > 1 else 0
        
        # For fast mode (7 descriptors): [4]=NumRotatableBonds, [5]=TPSA
        # For full mode (49 descriptors): [7]=NumRotatableBonds, [13]=TPSA  
        if n_descriptors == 7:
            rotatable = descriptors[:, 4] if n_descriptors > 4 else np.zeros(len(descriptors))
            tpsa = descriptors[:, 5] if n_descriptors > 5 else np.zeros(len(descriptors))
        else:
            rotatable = descriptors[:, 7] if n_descriptors > 7 else np.zeros(len(descriptors))
            tpsa = descriptors[:, 13] if n_descriptors > 13 else np.zeros(len(descriptors))
    ...
    def _plot_pharma_property_radar(self, ax, name: str, features: Dict):
        ...
        properties = {
            ...
            'Polarity': np.clip(np.mean(descriptors[:, 5] if n_descriptors == 7 else descriptors[:, 13]) / 140, 0, 1),  # TPSA
            'Flexibility': np.clip((10 - np.mean(descriptors[:, 4] if n_descriptors == 7 else descriptors[:, 7])) / 10, 0, 1),  # RotBonds
            'Complexity': np.clip((1000 - np.mean(descriptors[:, 0])) / 1000, 0, 1) if n_descriptors == 7 else np.clip((1000 - np.mean(descriptors[:, 16])) / 1000, 0, 1),  # BertzCT or MW
            'Aromaticity': np.clip(np.mean(descriptors[:, 6] if n_descriptors == 7 else descriptors[:, 9]) / 5, 0, 1)  # NumAromaticRings
        }
    ...
    def _create_admet_plots(self, name: str, features: Dict, path: Path):
        ...
        # Handle different descriptor modes
        n_descriptors = descriptors.shape[1] if len(descriptors.shape) > 1 else 0
        tpsa_idx = 5 if n_descriptors == 7 else 13
        if n_descriptors > tpsa_idx:
            axes[0,1].scatter(descriptors[:, 1], descriptors[:, tpsa_idx], alpha=0.5)
        else:
            axes[0,1].scatter(descriptors[:, 1], np.zeros(len(descriptors)), alpha=0.5)
        ...
        n_descriptors = descriptors.shape[1] if len(descriptors.shape) > 1 else 0
        rot_idx = 4 if n_descriptors == 7 else 7
        axes[1,0].scatter(descriptors[:, 0], descriptors[:, rot_idx] if n_descriptors > rot_idx else np.zeros(len(descriptors)), alpha=0.5)
    ...
    def _create_development_stage_plot(self, name: str, features: Dict, path: Path):
        ...
        lead_like = np.sum(
            (descriptors[:, 0] <= 350) & 
            (descriptors[:, 1] <= 3.5) & 
            (descriptors[:, 4 if n_descriptors == 7 else 7] <= 7)
        )
        drug_like = np.sum(
    ...
    def _create_bioavailability_plot(self, name: str, features: Dict, path: Path):
        ...
        for i in range(len(descriptors)):
            ...
            # Handle different descriptor indices for fast vs full mode
            if n_descriptors == 7:
                # Fast mode indices
                if descriptors[i, 2] <= 5: score += 1    # NumHDonors
                if descriptors[i, 3] <= 10: score += 1   # NumHAcceptors
                if descriptors[i, 4] <= 10: score += 1   # NumRotatableBonds
            else:
                # Full mode indices
                if n_descriptors > 7 and descriptors[i, 7] <= 5: score += 1    # NumHDonors
                if n_descriptors > 6 and descriptors[i, 6] <= 10: score += 1   # NumHAcceptors
                if n_descriptors > 4 and descriptors[i, 4] <= 10: score += 1   # NumRotatableBonds
            # TPSA check - handle different descriptor counts
            tpsa_idx = 5 if n_descriptors == 7 else 13
            if n_descriptors > tpsa_idx and descriptors[i, tpsa_idx] <= 140: score += 1  # TPSA
