...
class SummaryVisualizer:
    ...
    def _check_memory_status(self):
        ...
        try:
            ...
            if available_gb < 2:  # Less than 2GB available
                ...
                print("  [WARNING] Low memory mode enabled for visualizations")
        except ImportError:
            pass
    ...
    def create_all_summary_visualizations(self, datasets: Dict, splits: Dict,
        ...
        print("  [OK] Saved all individual plots from comprehensive view")
    ...
    def _create_mode_specific_summary_tables(self, datasets: Dict, ad_analysis: Dict):
        ...
        for mode in modes:
            try:
                ...
                print(f"    [OK] Created {mode} mode summary table")
                
            except Exception as e:
                print(f"    [ERROR] Failed to create {mode} mode summary table: {str(e)}")
                plt.close()
                gc.collect()
    ...
    def _create_main_summary_plot(self, datasets, splits, ad_analysis):
        ...
        try:
            ...
            # Create fig first! (this is important)
            ...
            print(f"    [OK] Saved comprehensive_summary.png")
            
        except Exception as e:
            print(f"    [ERROR] Failed to create main summary plot: {str(e)}")
            import traceback
            traceback.print_exc()
            plt.close('all')
            gc.collect()
        ...
        for mode in modes:
            try:
                ...
                if summary_data:
                    ...
                    print(f"    [OK] Created {mode} mode summary table (Excel)")
    ...
    def _plot_split_method_comparison(self, ax, ad_analysis: Dict):
        ...
        for dataset_ad in ad_analysis.values():
            for split_name, split_data in dataset_ad.items():
                if split_name in split_methods and split_data is not None:
                    if split_data is not None and 'ad_results' in split_data:
                        coverages = [r['coverage'] for r in split_data['ad_results'].values()
                                    if r and 'coverage' in r]
                        if coverages:
    ...
    def _plot_dataset_overview_summary(self, ax, datasets: Dict):
        ...
        # Use Unicode Greek characters (safer method)
        greek_letters = [
            '\u03B1', '\u03B2', '\u03B3', '\u03B4', '\u03B5', '\u03B6', 
            '\u03B7', '\u03B8', '\u03B9', '\u03BA', '\u03BB', '\u03BC'
        ]
        ...
        # greek_letters = ['alpha', 'beta', 'gamma', 'delta', 'epsilon', 'ζ', 'η', 'theta', 'ι', 'κ', 'lambda', 'mu']
        
        # Handle cases with more than 12 datasets
        if len(dataset_names) > 12:
            # Use numbers or extended labeling
    ...
    def _plot_decision_matrix(self, ax, datasets: Dict, ad_analysis: Dict):
        ...
        try:
            ...
            print("  [WARNING] Using fallback colormap")
        ...
        for i, name in enumerate(dataset_names):
            if decision_matrix[i, 3] >= 0.7:
                recommendations.append(f"{name}: [OK] Recommended")
            elif decision_matrix[i, 3] >= 0.5:
                recommendations.append(f"{name}: ~ Use with caution")
            else:
                recommendations.append(f"{name}: [X] Not recommended")
        ...
        for plot_name, plot_func in plots:
            try:
                ...
                safe_savefig(self.individual_path / f'{plot_name}.png',
                           dpi=PLOT_SETTINGS['figure_dpi'])
                print(f"    [OK] Saved {plot_name}.png")
            except Exception as e:
                print(f"    [ERROR] Failed to save {plot_name}.png: {str(e)}")
                plt.close()
                gc.collect()
...
QUALITY ASSESSMENT:
* {'[OK]' if np.mean(all_coverages) > 0.7 else '[X]'} Coverage > 70%
* {'[OK]' if total_samples > 5000 else '[X]'} Sufficient data (>5000 samples)
* {'[OK]' if n_train_test >= 3 else '[X]'} Multiple train/test datasets"""
