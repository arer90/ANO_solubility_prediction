...
try:
    ...
    print("[WARNING] Joblib not available, using ThreadPoolExecutor instead")
...
from .ad_methods import RegulatoryADMethods
try:
    from .advanced_ad_methods import AdvancedADMethods, ADEnsemble
    ADVANCED_AD_AVAILABLE = True
except ImportError:
    ADVANCED_AD_AVAILABLE = False
    print("[INFO] Advanced AD methods not available")
...
class ConcurrentQSARAnalyzer(BaseAnalyzer):
    def __init__(self, output_dir: str = "result/1_preprocess", 
                 ...
                 n_jobs: int = -1,
                 split_only_mode: bool = False,
                 update_data_folder: bool = True,
                 use_advanced_ad: bool = True):  # Changed default to True
        ...
        self.split_only_mode = split_only_mode
        self.update_data_folder = update_data_folder
        self.use_advanced_ad = True  # Always enable advanced AD (3D analysis)
        self.logger = logging.getLogger(__name__)
        
        # Initialize advanced AD methods if available
        if self.use_advanced_ad:
            self.advanced_ad_methods = AdvancedADMethods(ad_mode)
            self.ad_ensemble = ADEnsemble(ad_mode)
            print("[OK] Advanced AD methods enabled")
        
        # Parallel processing setup - Use conservative settings for memory management
        # Don't use all cores - leave room for system and memory management
        max_safe_workers = min(4, max(1, mp.cpu_count() // 4))  # Use 25% of cores, max 4
        self.n_jobs = n_jobs if n_jobs != -1 else max_safe_workers
        ...
        print(f"[LAUNCH] Concurrent QSAR Analyzer Initialized")
        print(f"[GRAPH] Performance Mode: {'[OK] Enabled' if performance_mode else '[X] Disabled'}")
        print(f"[TOOL] Parallel Jobs: {self.n_jobs}")
        print(f"[TOOL] Backend: {self.parallel_backend or 'ThreadPoolExecutor'}")
        print(f"[SAVE] Max Samples: {max_samples_analysis}")
        print(f"[SAVE] Memory Limit: {psutil.virtual_memory().total / (1024**3):.1f} GB")
        ...
        self.ad_methods = RegulatoryADMethods(
            random_state, ad_mode=ad_mode
        )
        ...
        for name in self.datasets.keys():
            try:
                ...
                if feature_data is not None:
                    ...
                    print(f" [OK] ({feature_data['features'].shape}) in {time.time()-start_time:.1f}s")
                else:
    ...
    def _calculate_features_safe(self):
        ...
        print("\n[CALC] Step 3: Calculating molecular features")
        ...
        if JOBLIB_AVAILABLE and self.parallel_backend and not self.performance_mode:
            try:
                ...
                print(f"    [WARNING] Parallel feature calculation failed: {str(e)}")
                print("    Falling back to sequential processing...")
                self._calculate_features_sequential()
        else:
            self._calculate_features_sequential()
        
        print(f"[OK] Features calculated for {len(self.features)} datasets")
        gc.collect()
    ...
    def _analyze_single_split_optimized(self, name: str, split_name: str, 
        ...
        try:
            ...
            # Check if split_data is valid
            if split_data is None:
                print(f"      [ERROR] {name}/{split_name}: split_data is None")
                return None
            
            # Get indices with safe access
            if 'train_idx' not in split_data or 'test_idx' not in split_data:
                print(f"      [ERROR] {name}/{split_name}: missing train_idx or test_idx in split_data")
                return None
                
            train_idx = split_data['train_idx']
            test_idx = split_data['test_idx']
            
            # Dynamic sampling - special handling for cluster split
            if split_name == 'cluster':
                # Cluster split allows larger test set
                max_train = 2000
                max_test = min(1000, len(test_idx))  # Allow up to 1000
            else:
                max_train = 2000
            ...
            if memory_percent > 80:
                ...
                print(f"      [WARNING] High memory ({memory_percent}%), reducing to {max_train}/{max_test}")
            ...
            # Calculate AD methods - pass actual sizes
            ad_results = self.ad_methods.calculate_all_methods(
                X_train, X_test
            )
            
            # Add actual size information to results
            if ad_results:
    ...
    def _perform_ad_analysis_safe(self):
        ...
        print(f"\n[LAB] Step 4: Performing AD analysis (mode: {self.ad_mode})")
        ...
        for i, (name, split_name, split_data, features) in enumerate(analysis_tasks):
            try:
                ...
                memory_percent = psutil.virtual_memory().percent
                if memory_percent > 80:
                    print(f"    [WARNING] High memory usage: {memory_percent}%, running cleanup...")
                    gc.collect()
                    time.sleep(1)
                ...
                print(f"    [ERROR] {name}/{split_name} failed: {str(e)}")
                self.ad_analysis[name][split_name] = None
        
        print(f"    [OK] Completed {len(analysis_tasks)} AD analyses")
    
    def set_existing_splits(self, existing_splits: Dict):
        """Set existing splits for analyze-only mode"""
        print("\n[INFO] Setting existing splits for analysis")
        for dataset_name in existing_splits:
            if dataset_name in self.datasets:
                ...
                # Use existing column names from data
                data = self.datasets[dataset_name]['data']
                if 'smiles' in data.columns:
                    self.splits[dataset_name]['smiles'] = data['smiles'].to_list()
                    self.splits[dataset_name]['targets'] = data['target'].to_numpy()
                elif 'target_x' in data.columns:
                    self.splits[dataset_name]['smiles'] = data['target_x'].to_list()
                    self.splits[dataset_name]['targets'] = data['target_y'].to_numpy()
                
                self.splits[dataset_name]['splits'] = existing_splits[dataset_name]
                print(f"  Set {len(existing_splits[dataset_name])} splits for {dataset_name}")
    ...
    def analyze_datasets(self, df_dict: Dict[str, pl.DataFrame], 
        ...
        if self.split_only_mode:
            print("\n[LAUNCH] STARTING SPLIT-ONLY MODE")
            print("=" * 60)
            print("[FAST] FAST MODE: Only creating data splits, skipping all analysis")
        elif skip_split_creation:
            print("\n[LAUNCH] STARTING ANALYSIS-ONLY MODE (using existing splits)")
            print("=" * 60)
        else:
            print("\n[LAUNCH] STARTING CONCURRENT QSAR ANALYSIS")
            print("=" * 60)
        ...
        try:
            ...
            with self.performance_tracker.track("Total Analysis"):
                ...
                # Step 2: Create and save splits (skip if using existing splits)
                if not skip_split_creation:
                    ...
                    # If using existing splits, set them now
                    print("\n[INFO] Using existing splits from loaded data")
                    if existing_splits:
                        self.set_existing_splits(existing_splits)
                    
                self._cleanup_intermediate_data()
                
                # SPLIT-ONLY MODE: Skip everything after splitting
                if self.split_only_mode:
                    # Update data folder with train/test splits
                    if self.update_data_folder:
                        self._update_data_folder_with_splits()
                    
                    print("\n" + "="*60)
                    print("[OK] SPLIT-ONLY MODE COMPLETED!")
                    print("="*60)
                    print(f"All splits saved to: {self.output_dir}/")
                    if self.update_data_folder:
                        print(f"Data folder updated: data/train/ and data/test/")
                    print("Generated outputs:")
                    print("  - Train/test splits for all strategies")
                    print("  - Split reports and statistics")
                    if self.update_data_folder:
                        print("  - Updated data/train/ and data/test/ folders")
                    print("[FAST] Analysis completed in FAST mode!")
                    return self
                ...
                if ad_analysis_mode == 'all':
                    print("\n" + "="*60)
                    print("[CHART] Running analysis for ALL AD modes")
                    print("="*60)
                    ...
                    for mode in ['strict', 'flexible', 'adaptive']:
                        try:
                            ...
                            with self.performance_tracker.track(f"Visualization ({mode})"):
                                ...
                                # AD visualizations already called inside _create_visualizations_sequential
                            ...
                            print(f"[WARNING] Error in {mode} mode: {str(e)[:100]}")
                            print(f"[INFO] Continuing with next mode...")
                            self.all_mode_results[mode] = {
                                'error': str(e),
                                'mode_info': AD_COVERAGE_MODES.get(mode, {
                                    'name': mode.title() + ' Mode',
                                    'reference': 'Error occurred during analysis'
                                }),
                                'ad_analysis': self.ad_analysis.copy() if hasattr(self, 'ad_analysis') else {}
                            }
                            continue
                    
                    # Create mode comparison visualizations
                    print("\n" + "="*50)
                    print("[CHART] Creating mode comparison visualizations")
                    print("="*50)
                    
                    with self.performance_tracker.track("Mode Comparison"):
                        # Mode comparison disabled - not generating useful output
                        # self._create_mode_comparison_visualizations()
                        pass
                        self._create_all_modes_summary_report()
                    
                    # Update data folder with final splits (for 'all' mode)
                    if self.update_data_folder:
                        with self.performance_tracker.track("Updating Data Folder"):
                            self._update_data_folder_with_splits()
                    ...
                    # Step 8: Update data folder with final splits
                    if self.update_data_folder:
                        with self.performance_tracker.track("Updating Data Folder"):
                            self._update_data_folder_with_splits()
                        
                        self._track_memory("After data folder update")
    ...
    def _create_mode_comparison_visualizations(self):
        ...
        for mode, results in self.all_mode_results.items():
            coverages = []
            # Skip modes with errors
            if 'error' in results or 'ad_analysis' not in results:
                continue
        ...
        mode_info_text = ""
        for mode in modes:
            mode_info = self.all_mode_results[mode].get('mode_info', {
                'name': f'{mode.title()} Mode',
                'reference': 'Mode information not available'
            })
            mode_info_text += f"{mode.upper()} MODE:\n"
            mode_info_text += f"  Name: {mode_info.get('name', mode.title())}\n"
            ref = mode_info.get('reference', 'N/A')
            mode_info_text += f"  Reference: {ref[:50] if len(ref) > 50 else ref}...\n\n"
        ...
        print("  [OK] Mode comparison visualizations created")
    ...
    def _create_all_modes_summary_report(self):
        ...
        with open(report_path, 'w') as f:
            ...
            for mode, results in self.all_mode_results.items():
                ...
                f.write(f"\n{mode.upper()} MODE:\n")
                f.write(f"  Full Name: {mode_info.get('name', mode.title())}\n")
                f.write(f"  Reference: {mode_info.get('reference', 'N/A')}\n")
            ...
            if mode_means.get('strict', 0) >= 0.9:
                f.write("\n[OK] STRICT mode meets regulatory requirements (>=90% coverage)\n")
                f.write("  Recommended for: Regulatory submissions, safety-critical applications\n")
            else:
                f.write("\n[X] STRICT mode does not meet regulatory requirements\n")
            ...
            f.write(f"\n[OK] {best_research_mode.upper()} mode recommended for research\n")
        ...
        print(f"  [OK] All modes summary report saved: {report_path.name}")
    
    def _load_and_preprocess_data(self, df_dict: Dict[str, pl.DataFrame], 
                                 test_only_datasets: List[str]):
        """Load and preprocess data"""
        print("\n[CHART] Step 1: Loading and preprocessing data")
    ...
    def _create_and_save_splits(self):
        ...
        print("\n[FOLDER] Step 2: Creating and saving data splits")
        
        # Import ACTIVE_SPLIT_TYPES from config
        try:
            from config import ACTIVE_SPLIT_TYPES
            print(f"  [CONFIG] Using active split types: {ACTIVE_SPLIT_TYPES}")
        except ImportError:
            ACTIVE_SPLIT_TYPES = None
            print("  [CONFIG] No ACTIVE_SPLIT_TYPES configured, creating all splits")
        ...
        if tqdm_available:
            dataset_iter = tqdm(dataset_items, desc="Creating splits for datasets")
        else:
            dataset_iter = dataset_items
            
        for name, dataset_info in dataset_iter:
            # Check if splits already exist
            if 'splits' in self.splits[name] and self.splits[name]['splits']:
                print(f"  [CACHE] Splits already exist for {name}, skipping...")
                continue
            ...
            if tqdm_available:
                dataset_iter.set_description(f"Creating splits for {name}")
            else:
                print(f"  [SPLIT] Creating splits for {name}...")
            
            # Create splits
            # Note: DataSplitter doesn't support active_split_types parameter
            # It creates all splits by default
            ...
            if not tqdm_available:
                print(f"  [OK] Splits created for {name}")
    ...
    def analyze_ad_performance_for_all_datasets(self, features_dict: Dict, 
                                            ...
                                            splits_dict: Dict):
        """AD performance analysis - modified targets passing"""
    ...
    def _calculate_features_parallel(self):
        ...
        for name, feature_data in results:
            if feature_data is not None:
                ...
                print(f"    [OK] {name}: {feature_data['features'].shape}")
    ...
    def _create_visualizations_sequential(self, ad_analysis_mode: str):
        ...
        # 2. Meta Visualizations - Need to add!
        ...
        # 3. Statistical Visualizations - Need to add!
        ...
        # 4. Summary Visualizations - Need to add!
        ...
        if hasattr(self, 'ad_visualizer') and hasattr(self.ad_visualizer, 'performance_analyzer'):
            if self.ad_visualizer.performance_analyzer:
                try:
                    print("  [OK] AD Performance Analyzer is available")
                    ...
                    if ad_perf_results:
                        print(f"  [OK] AD performance analysis completed")
                    else:
                        print("  [WARNING] AD performance analysis returned no results")
                        
                except Exception as e:
                    print(f"  [ERROR] AD performance analysis failed: {str(e)}")
                    import traceback
                    traceback.print_exc()
            else:
                print("  [WARNING] AD Performance Analyzer not available")
        else:
            print("  [WARNING] ad_visualizer or performance_analyzer not found")
    ...
    def _generate_final_report(self):
        ...
        print("\n" + "="*60)
        print("[CHART] ANALYSIS MODE INFORMATION:")
    ...
    def _update_data_folder_with_splits(self):
        """Update data folder with train/test splits while preserving existing files"""
        print("\n[UPDATE] Step: Updating data folder with train/test splits")
        ...
        # Create data/train and data/test directories if they don't exist
        data_train_dir.mkdir(parents=True, exist_ok=True)
        data_test_dir.mkdir(parents=True, exist_ok=True)
        # Create 'to' folder for test-only datasets
        (data_test_dir / "to").mkdir(parents=True, exist_ok=True)
        
        # Source directories (where splits are currently saved)
        source_train_dir = Path(self.output_dir) / "train"
        source_test_dir = Path(self.output_dir) / "test"
        
        if not source_train_dir.exists() or not source_test_dir.exists():
            print("  [WARNING] No splits found to update data folder")
            return
        
        # Copy train splits to data/train (preserving existing files)
        if source_train_dir.exists():
            print(f"  [COPY] Updating data/train/ from {source_train_dir}")
            self._copy_splits_preserving_existing(source_train_dir, data_train_dir, "train")
        
        # Copy test splits to data/test (preserving existing files)
        if source_test_dir.exists():
            print(f"  [COPY] Updating data/test/ from {source_test_dir}")
            self._copy_splits_preserving_existing(source_test_dir, data_test_dir, "test")
        
        print("  [OK] Data folder updated successfully")
        print(f"    - Train splits: {data_train_dir}")
        print(f"    - Test splits: {data_test_dir}")
    
    def _copy_splits_preserving_existing(self, source_dir: Path, target_dir: Path, split_type: str):
        """Copy splits while preserving existing files"""
        import shutil
        
        copied_count = 0
        preserved_count = 0
        
        # Walk through all subdirectories and files in source
        for source_path in source_dir.rglob("*"):
            if source_path.is_file():
                ...
                if target_path.exists():
                    # Compare file sizes or timestamps to decide whether to update
                    source_stat = source_path.stat()
                    target_stat = target_path.stat()
                    
                    # Update if source is newer or different size
                    if (source_stat.st_mtime > target_stat.st_mtime or 
                        ...
                        preserved_count += 1
                        print(f"    [KEEP] {rel_path} (already up-to-date)")
                else:
                    # Copy new file
                    shutil.copy2(source_path, target_path)
                    copied_count += 1
        ...
        print(f"  [SUMMARY] {split_type}: {copied_count} files updated/added, {preserved_count} files preserved")
    
    def _analyze_single_split_memory_optimized(self, name: str, split_name: str, 
                                          split_data: dict, features: np.ndarray):
        """Memory-optimized single split AD analysis"""
        try:
            if split_name == 'test_only':
                return self._perform_test_only_ad_analysis(name, split_data, features)
            ...
            if memory_percent > 80:
                max_train = 1000
                max_test = 300
                print(f"      [WARNING] High memory usage ({memory_percent}%), reducing samples")
            ...
            # Calculate AD methods
            ad_results = self.ad_methods.calculate_all_methods(
                X_train, X_test
            )
            
            # Add advanced AD methods if enabled
            if self.use_advanced_ad:
                try:
                    # Create ensemble combining standard and advanced methods
                    ensemble_results = self.ad_ensemble.create_ensemble_ad(
                        X_train, X_test, ad_results
                    )
                    # Update results with advanced methods
                    ad_results.update(ensemble_results)
                    print(f"        [OK] Advanced AD methods added")
                except Exception as e:
                    print(f"        [WARNING] Advanced AD failed: {str(e)[:50]}")
            
            # Memory cleanup
            del X_train, X_test
            gc.collect()
            
            # Consensus AD (simple version)
            consensus_ad = self._calculate_simple_consensus(ad_results)
    ...
    def _perform_test_only_ad_analysis(self, name: str, split_data: dict, 
        ...
        # Check if split_data is valid
        if split_data is None:
            print(f"      [ERROR] {name}: split_data is None for test_only analysis")
            return None
            
        if 'test_idx' not in split_data:
            print(f"      [ERROR] {name}: missing test_idx in split_data for test_only analysis")
            return None
...
class MemoryMonitor:
    ...
    def checkpoint(self, label: str, force_gc: bool = False):
        ...
        if current_memory > self.threshold_mb:
            warning = f"[WARNING] High memory usage at {label}: {current_memory:.1f} MB"
            print(warning)
            ...
            if new_memory < current_memory:
                print(f"   [OK] Freed {current_memory - new_memory:.1f} MB")
    ...
    def print_report(self):
        ...
        # Print warnings
        if self.warnings:
            print(f"\n[WARNING] Memory warnings ({len(self.warnings)}):")
