...
# Try to import config.py, use defaults if failed
try:
    ...
    # Default print_system_recommendations function
    def print_system_recommendations():
        print("\n" + "="*60)
        print("[SYSTEM]  SYSTEM INFO (Default)")
        print("="*60)
...
class AdvancedDataSplitter:
    ...
    SPLIT_CRITERIA = {
        ...
        'chemical_space_coverage': {
            ...
            'radius': 3,
            'structure_weight': 0.7,  # alpha in distance metric
            'property_weight': 0.3,   # 1-alpha
        ...
        'activity_cliff': {
            ...
            'smoothing_factor': 0.001,  # epsilon in SALI formula
    ...
    def _manage_cache_size(self):
        ...
        if len(self._cache) >= self._cache_max_size:
            ...
            self.logger.info(f"[CACHE] Removed oldest cache entry to maintain size limit")
    
    def clear_cache(self):
        """Clear all cached splits"""
        self._cache.clear()
        self.logger.info("[CACHE] All cached splits cleared")
    ...
    def _initialize_rdkit_components(self):
        ...
        # Initialize unsupported elements for 3D conformer generation
        # Same as chem_descriptor_maker.py - comprehensive list of elements that fail in ETKDG/UFF/MMFF
        self.unsupported_3d_elements = {
    ...
    def _can_generate_3d_conformer(self, smiles: str) -> Tuple[bool, str]:
        """
        Check if a SMILES can generate 3D conformer.
        Matches the logic from chem_descriptor_maker.py to prevent mismatches.
        ...
        try:
            ...
            # Check for unsupported elements (same as chem_descriptor_maker.py)
            atoms = mol.GetAtoms()
            for atom in atoms:
                symbol = atom.GetSymbol()
                if symbol in self.unsupported_3d_elements:
                    return False, f"Contains unsupported element: {symbol}"

            # Try to generate 3D conformer (same method as chem_descriptor_maker.py)
            mol_copy = Chem.Mol(mol)
            mol_copy = AllChem.AddHs(mol_copy)
            ...
            try:
                confIds = AllChem.EmbedMultipleConfs(mol_copy, numConfs=1, params=params)
                if len(confIds) == 0:
                    return False, "3D embedding failed"

                # Try UFF optimization
                try:
                    AllChem.UFFOptimizeMolecule(mol_copy, confId=0, maxIters=200)
                except:
                    return False, "UFF optimization failed"

                return True, "OK"

            except Exception as e:
                return False, f"3D generation error: {str(e)}"

        except Exception as e:
            return False, f"Validation error: {str(e)}"

    def filter_3d_incompatible_molecules(self,
                                         smiles: List[str],
                                         targets: np.ndarray,
                                         dataset_name: str = "dataset") -> Tuple[List[str], np.ndarray, Dict]:
        """
        Filter out molecules that cannot generate 3D conformers.
        This prevents mismatches between fingerprints and 3D descriptors in later modules.

        Args:
            smiles: List of SMILES strings
            targets: Array of target values (log S)
            dataset_name: Name of dataset for reporting

        Returns:
            Tuple of (filtered_smiles, filtered_targets, report_dict)
        """
        if not RDKIT_AVAILABLE:
            self.logger.warning("RDKit not available, skipping 3D conformer validation")
            return smiles, targets, {'filtered': 0, 'total': len(smiles)}

        self.logger.info(f"Validating 3D conformer generation for {len(smiles)} molecules...")

        valid_indices = []
        invalid_molecules = []

        for idx, smi in enumerate(smiles):
            can_generate, reason = self._can_generate_3d_conformer(smi)
            ...
            # Progress logging
            if (idx + 1) % 100 == 0:
                self.logger.info(f"  Validated {idx + 1}/{len(smiles)} molecules "
                               f"({len(valid_indices)} valid, {len(invalid_molecules)} invalid)")

        # Filter arrays
        filtered_smiles = [smiles[i] for i in valid_indices]
        ...
        report = {
            ...
            'filter_ratio': len(valid_indices) / len(smiles) if len(smiles) > 0 else 0,
            'invalid_details': invalid_molecules
        }

        # Log summary
        self.logger.info(f"\n3D Conformer Validation Summary:")
        self.logger.info(f"  Total molecules: {len(smiles)}")
        self.logger.info(f"  Valid (can generate 3D): {len(valid_indices)} ({report['filter_ratio']*100:.1f}%)")
        self.logger.info(f"  Invalid (cannot generate 3D): {len(invalid_molecules)} ({(1-report['filter_ratio'])*100:.1f}%)")

        if invalid_molecules:
            # Count reasons
            reason_counts = {}
            for inv in invalid_molecules:
                reason = inv['reason']
                reason_counts[reason] = reason_counts.get(reason, 0) + 1

            self.logger.info(f"\n  Failure reasons:")
            for reason, count in sorted(reason_counts.items(), key=lambda x: x[1], reverse=True):
                self.logger.info(f"    - {reason}: {count} molecules")

        # Save detailed report
        self._save_3d_filter_report(report, dataset_name)

        return filtered_smiles, filtered_targets, report

    def _save_3d_filter_report(self, report: Dict, dataset_name: str):
        """Save 3D conformer filtering report to JSON"""
        try:
            report_path = self.reports_dir / f'{dataset_name}_3d_filter_report.json'

            with open(report_path, 'w') as f:
                json.dump(report, f, indent=2)

            self.logger.info(f"  Saved 3D filter report to: {report_path}")

            # Also save simple CSV of invalid molecules
            if report['invalid_details']:
                csv_path = self.reports_dir / f'{dataset_name}_3d_invalid_molecules.csv'
                df = pd.DataFrame(report['invalid_details'])
                df.to_csv(csv_path, index=False)
                self.logger.info(f"  Saved invalid molecules list to: {csv_path}")

        except Exception as e:
            self.logger.warning(f"Failed to save 3D filter report: {e}")
    ...
    def create_all_splits(self, 
        ...
        cache_key = f"{name}_{len(smiles)}_{hash(tuple(smiles))}_{hash(tuple(targets))}_{is_test_only}"
        
        # Check if splits already exist in cache
        if cache_key in self._cache:
            self.logger.info(f"\n[CACHE] Using cached splits for {name}")
            self.logger.info(f"Dataset size: {len(smiles)} molecules (cached)")
            return self._cache[cache_key]
        
        self.logger.info(f"\nCreating advanced splits for {name} (v{self.version})...")
        self.logger.info(f"Original dataset size: {len(smiles)} molecules")

        # IMPORTANT: Filter molecules that cannot generate 3D conformers FIRST
        # This prevents mismatches between fingerprints and 3D descriptors in downstream modules
        self.logger.info("\n=== Step 1: Filtering molecules by 3D conformer compatibility ===")
        smiles, targets, filter_report = self.filter_3d_incompatible_molecules(
            smiles, targets, dataset_name=name
        )
        self.logger.info(f"After 3D filtering: {len(smiles)} molecules remaining")

        if len(smiles) == 0:
            raise ValueError(f"No molecules remaining after 3D conformer filtering!")
        ...
        self.split_reports[name] = {
            'dataset_info': {
                ...
                '3d_filter_report': filter_report  # Add 3D filtering info
            },
            'splits': {}
        }
        ...
        self.logger.info(f"[CACHE] Splits cached for {name} (cache size: {len(self._cache)})")
    ...
    def _calculate_solubility_statistics(self, targets: np.ndarray) -> Dict:
        ...
        stats = {
            ...
            'distribution': {
                'very_poor': int(np.sum(targets < -6)),      # < 1 muM
                'poor': int(np.sum((targets >= -6) & (targets < -4))),       # 1 muM - 100 muM
                'moderate': int(np.sum((targets >= -4) & (targets < -2))),   # 100 muM - 10 mM
    ...
    def _create_chemical_space_coverage_split(self, name: str, smiles: List[str], 
        ...
        Distance metric:
        D(i,j) = alpha·D_tanimoto(FP_i, FP_j) + (1-alpha)·D_euclidean(P_i, P_j)
        where alpha = 0.7 (structure weight)
        ...
        try:
            ...
            # Increase iteration limit for larger datasets
            max_iterations = min(target_test_size * 3, len(remaining_idx) + 100)
            iteration_count = 0
            
            # Dynamic timeout based on dataset size
            timeout_seconds = min(600, max(300, len(smiles) // 10))  # 5-10 minutes max
            start_time = time.time()
            
            while len(test_idx) < target_test_size and remaining_idx and iteration_count < max_iterations:
                # Check timeout
                if time.time() - start_time > timeout_seconds:
                    self.logger.warning(f"MaxMin selection timeout after {timeout_seconds} seconds")
                    break
                iteration_count += 1
                ...
                # Safety check for empty min_distances
                if not min_distances:
                    self.logger.warning("No distances calculated, breaking loop")
                    break
                
                # Additional safety: if no progress for 10 iterations
                if iteration_count > 10 and len(test_idx) == len(test_idx):
                    prev_test_size = len(test_idx)
                    if hasattr(self, '_prev_test_size') and self._prev_test_size == prev_test_size:
                        self._no_progress_count = getattr(self, '_no_progress_count', 0) + 1
                        if self._no_progress_count > 10:
                            self.logger.warning("No progress for 10 iterations, breaking loop")
                            break
                    else:
                        self._no_progress_count = 0
                    self._prev_test_size = prev_test_size
            ...
            # Log if loop was terminated due to safety counter
            if iteration_count >= max_iterations:
                self.logger.warning(f"MaxMin selection terminated due to safety counter at {iteration_count} iterations")
    ...
    def _create_activity_cliff_split(self, name: str, smiles: List[str], 
        ...
        SALI(i,j) = |A_i - A_j| / (1 - sim(i,j) + epsilon)
        
        where:
        - A_i, A_j: Activities (log S values)
        - sim(i,j): Tanimoto similarity
        - epsilon: Small smoothing factor (0.001)
    ...
    def _create_ensemble_split(self, name: str, smiles: List[str], 
        ...
        maximize Quality = alpha·DistQuality + beta·RangeQuality + gamma·SizeQuality
    ...
    def _select_diverse_subset_enhanced(self, fps: List, candidates: List[int],
        ...
        # MaxMin selection with safety counter and timeout
        max_iterations = min(n_select * 2, len(candidates) + 50)
        iteration_count = 0
        start_time = time.time()
        timeout_seconds = 300  # 5 minutes max
        
        while (len(selected) < n_select and candidates and 
            ...
            for candidate in candidates:
                ...
                min_dist = min(combined_dists) if combined_dists else 0
            ...
            if best_candidate is not None:
                ...
                # Safety fallback: select random candidate if no best found
                if candidates:
        ...
        # Log if loop was terminated due to safety measures
        if iteration_count >= max_iterations:
            print(f"[WARNING] MaxMin selection terminated due to iteration limit at {iteration_count} iterations")
        elif (time.time() - start_time) >= timeout_seconds:
            print(f"[WARNING] MaxMin selection terminated due to timeout after {timeout_seconds} seconds")
    ...
    def _create_distant_cluster_split(self, features: np.ndarray,
        ...
        # Add safety counter and timeout to prevent infinite loops
        max_iterations = min(len(unassigned) * 2, 200)
        iteration_count = 0
        start_time = time.time()
        timeout_seconds = 300  # 5 minutes max
        
        while (unassigned and iteration_count < max_iterations and 
               (time.time() - start_time) < timeout_seconds):
            iteration_count += 1
            best_cluster = None
            ...
            # Fallback assignment - ensure we always select something
            if best_cluster is None and unassigned:
                best_cluster = unassigned[0]
                best_assignment = 'train' if train_size < test_size else 'test'
            
            # Safety check
            if best_cluster is None:
                print(f"[ERROR] No cluster selected, breaking loop. Remaining unassigned: {len(unassigned)}")
                break
        ...
        # Log if loop was terminated due to safety measures
        if iteration_count >= max_iterations:
            print(f"[WARNING] Cluster assignment terminated due to iteration limit at {iteration_count} iterations")
        elif (time.time() - start_time) >= timeout_seconds:
            print(f"[WARNING] Cluster assignment terminated due to timeout after {timeout_seconds} seconds")
