...
def monitor_memory(func):
    ...
    This decorator tracks memory allocation and peak usage during function calls,
    helping to identify memory-intensive operations and potential memory leaks.
...
def load_data(data_folder: str = "data", verbose: bool = False, details: bool = False, 
    ...
    """
    Optimized data loader - reads all molecular dataset files and returns clean DataFrames.
    
    This function automatically scans the data directory for CSV files containing
    molecular data (SMILES and solubility values), loads them efficiently using
    Polars DataFrame library, and handles various encoding formats.
    
    Key Features:
    - Automatic encoding detection for different file formats
    - Parallel processing for multiple files
    - Memory-efficient chunk processing for large files
    - Automatic column name standardization
    - Skips train/test directories to avoid loading pre-split data
    
    Parameters:
    -----------
    data_folder : str
        Target folder containing molecular dataset files (default: "data")
    verbose : bool
        Show detailed processing information (default: False)
    details : bool
        Show detailed file information including sizes (default: False)
    parallel : bool
        Use parallel processing for multiple files (default: True)
    chunk_size : int
        Number of rows to process at once for large files (default: 100000)
    
    Returns:
    --------
    Dict[str, pl.DataFrame]
        Dictionary mapping dataset names to Polars DataFrames
        Keys: filename without extension (e.g., 'ws496_logS', 'delaney-processed')
        Values: DataFrames with standardized columns [smiles, target]
    
    Example:
    --------
    >>> df_dict = load_data(verbose=True)
    >>> ws_data = df_dict['ws496_logS']
    >>> print(ws_data.columns)
    ['smiles', 'target']
    """
    ...
    # Get supported files (excluding train/test directories)
    ...
    if verbose:
        print(f"Found {len(files)} files (train/test directories excluded)")
        if details:
            for f in files:
                size_mb = f.stat().st_size / (1 << 20)  # Accurate MB conversion
...
def _get_files(data_path: Path, verbose: bool) -> List[Path]:
    """Get supported files from data folder, excluding train/test directories"""
    if not data_path.exists():
        raise FileNotFoundError(f"Data folder not found: {data_path}")
    
    supported_extensions = {'.csv', '.tsv', '.txt', '.xlsx', '.xls', '.json', '.parquet'}
    excluded_dirs = {'train', 'test', 'Train', 'Test', 'TRAIN', 'TEST'}  # Case-insensitive exclusion
    files = []
    
    for file_path in data_path.iterdir():
        # Skip train/test directories
        if file_path.is_dir() and file_path.name in excluded_dirs:
            if verbose:
...
def _print_summary(df_dict: Dict[str, pl.DataFrame], success: int, failed: int, 
    ...
    print(f"Note: train/test directories excluded from loading")
    ...
    if verbose or details:
        ...
        for name, df in sorted(df_dict.items()):
            ...
            print(f"    target_x: {'YES' if has_x else 'NO'} ({df['target_x'].drop_nulls().len()} values)")
            print(f"    target_y: {'YES' if has_y else 'NO'} ({df['target_y'].drop_nulls().len()} values)")
