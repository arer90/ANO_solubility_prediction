...
class QSARMetrics:
    ...
    def evaluate_inner_outer_difference(self, inner_metrics: Dict, 
        ...
        if rmse_difference > 0.5:  # More than 0.5 log units difference
            warnings.append("Large RMSE difference (>0.5 log units) - model may be overfitting")
        if rmse_relative_diff > 30:  # More than 30% relative difference
            warnings.append("High relative difference (>30%) - poor generalization")
        if r2_difference < -0.2:  # R² drops by more than 0.2
            warnings.append("Significant R² drop - model performance degradation")
