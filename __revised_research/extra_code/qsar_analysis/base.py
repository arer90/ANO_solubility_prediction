...
try:
    ...
    print("⚠️ RDKit not available. Using simulated features.")
    ...
    def validate_data(self, df: pl.DataFrame, name: str) -> bool:
        ...
        for col in required_columns:
            if col not in df.columns:
                print(f"❌ Dataset '{name}' missing required column: {col}")
                return False
        
        # Check for null values
        null_counts = df.null_count()
        if null_counts['target_x'][0] > 0 or null_counts['target_y'][0] > 0:
            print(f"⚠️ Dataset '{name}' contains null values")
        
        # Check minimum size
        if len(df) < 10:
            print(f"❌ Dataset '{name}' has insufficient samples ({len(df)})")
            return False
