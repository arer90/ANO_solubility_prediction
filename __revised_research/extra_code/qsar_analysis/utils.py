...
def generate_decision_report(output_dir: Path, ad_analysis: Dict, 
    ...
    with open(report_path, 'w') as f:
        ...
        f.write("|----------|-------|----------|\n")
        f.write(f"| ✅ Recommended | {len(decisions['recommended'])} | {', '.join(decisions['recommended'])} |\n")
        f.write(f"| ⚠️ Caution | {len(decisions['caution'])} | {', '.join(decisions['caution'])} |\n")
        f.write(f"| ❌ Not Recommended | {len(decisions['not_recommended'])} | {', '.join(decisions['not_recommended'])} |\n")
...
def assess_dataset_quality(mean_coverage: float, min_similarity: float, 
    ...
    if ad_mode == 'strict':
        if coverage_quality.lower() == 'excellent' and similarity_quality in ['excellent', 'good']:
            return 'EXCELLENT - READY FOR REGULATORY', '✅'
        ...
        if coverage_quality.lower() in ['excellent', 'good'] and similarity_quality in ['excellent', 'good', 'acceptable']:
            return 'SUITABLE FOR APPLICATION', '✅'
        elif coverage_quality.lower() == 'acceptable':
            return 'ACCEPTABLE WITH LIMITATIONS', '⚠️'
        else:
            return 'NOT SUITABLE', '❌'
...
def print_analysis_summary(start_time: float, datasets: Dict, 
    ...
    print("\n" + "="*60)
    print("🎉 ENHANCED QSAR ANALYSIS COMPLETE!")
    print("="*60)
