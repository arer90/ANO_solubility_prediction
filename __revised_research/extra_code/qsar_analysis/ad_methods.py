...
class RegulatoryADMethods:
    ...
    def calculate_all_methods(self, X_train: np.ndarray, X_test: np.ndarray, 
        ...
        # Define regulatory-approved methods
        regulatory_methods = [
            # Tier 1: Most widely accepted
            ('leverage', self.calculate_leverage),
            ('euclidean_distance', self.calculate_euclidean_distance),
            ('descriptor_range', self.calculate_descriptor_range),
            
            # Tier 2: Requires additional computation
            ('knn_distance', self.calculate_knn_distance),
            ('dmodx', self.calculate_dmodx),
        ]
        
        # Add standardized residuals if predictions available
        if y_test is not None and y_pred_test is not None:
            regulatory_methods.insert(1, ('standardized_residuals', 
                                        lambda X_tr, X_te: self.calculate_standardized_residuals(
                                            y_test, y_pred_test)))
        ...
        # Calculate Williams plot if both leverage and residuals available
    ...
    def calculate_dmodx(self, X_train: np.ndarray, X_test: np.ndarray) -> Optional[Dict]:
        """
        Distance to Model in X-space (DModX) - ECHA approved
        For PCA-based models
        
        Formula: DModX = sqrt(Σe²/(M-A))
        where e = residuals, M = variables, A = components
        
        References:
        - ECHA (2016) Practical Guide 5
        - Eriksson et al. (2003) Environ Health Perspect 111:1361-1375
        """
        try:
            # Determine number of components (95% variance)
            ...
            # Calculate residuals
            train_residuals = X_train - X_train_reconstructed
            test_residuals = X_test - X_test_reconstructed
            
            # Calculate DModX
            M = X_train.shape[1]  # number of variables
            A = pca.n_components_  # number of components
            
            train_dmodx = np.sqrt(np.sum(train_residuals**2, axis=1) / (M - A))
            test_dmodx = np.sqrt(np.sum(test_residuals**2, axis=1) / (M - A))
            
            # Critical value based on F-distribution
            # Dcrit = F_crit * sqrt(2)
            alpha = 0.05
            f_crit = stats.f.ppf(1 - alpha, M - A, (len(X_train) - A - 1) * (M - A))
            threshold = np.sqrt(f_crit * 2)
            
            # Alternative: 95th percentile of training DModX
            threshold_percentile = np.percentile(train_dmodx, 95)
            
            # Use the more conservative threshold
            final_threshold = max(threshold, threshold_percentile)
            
            # AD membership
            in_ad = test_dmodx <= final_threshold
            coverage = float(np.mean(in_ad))
    ...
    def calculate_consensus_ad(self, ad_results: Dict) -> Dict:
        ...
        """
        # Collect results from regulatory methods only
        regulatory_methods = ['leverage', 'standardized_residuals', 'williams_plot',
                            'knn_distance', 'euclidean_distance', 'descriptor_range', 'dmodx']
    ...
    def _get_regulatory_weights(self, method_names: List[str]) -> np.ndarray:
        ...
        regulatory_importance = {
            'leverage': 1.0,           # All agencies
            'williams_plot': 1.0,      # FDA, ECHA
            'standardized_residuals': 0.9,  # FDA, ECHA
...
class ConcurrentADMethods(RegulatoryADMethods):
    ...
    def _calculate_all_methods_parallel(self, X_train: np.ndarray, X_test: np.ndarray, 
        ...
        # Define methods
        methods = {
            'leverage': (self.calculate_leverage, (X_train_proc, X_test_proc)),
            ...
            'dmodx': (self.calculate_dmodx, (X_train_proc, X_test_proc)),
        }
