...
class VisualizationManager:
    ...
    def create_all_visualizations(self, datasets: Dict, splits: Dict, features: Dict, 
        ...
        if not self._validate_inputs(datasets, splits, features, ad_analysis, statistical_results):
            print("\n⚠️ Input validation failed. Some visualizations may be skipped.")
        ...
        try:
            ...
            print("  ✓ AD visualizations completed")
        except Exception as e:
            print(f"  ❌ AD visualizations failed: {str(e)}")
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            print("  ✓ Meta visualizations completed")
        except Exception as e:
            print(f"  ❌ Meta visualizations failed: {str(e)}")
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            print(f"  ❌ Statistical visualizations failed: {str(e)}")
            import traceback
            traceback.print_exc()
        ...
        try:
            ...
            print("  ✓ Summary visualizations completed")
        except Exception as e:
            print(f"  ❌ Summary visualizations failed: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def _validate_inputs(self, datasets: Dict, splits: Dict, features: Dict,
        ...
        # Check AD analysis
        if not ad_analysis:
            print("  ⚠️ No AD analysis results provided")
            valid = False
        else:
            print(f"  ✓ AD Analysis: {len(ad_analysis)}")
            
        # Check statistical results
        if not statistical_results:
            print("  ⚠️ No statistical results provided")
            valid = False
        else:
            print(f"  ✓ Statistical Results: {len(statistical_results)}")
        ...
        if datasets and splits:
            ...
            if dataset_names != split_names:
                print(f"  ⚠️ Dataset names mismatch: {dataset_names - split_names}")
                valid = False
